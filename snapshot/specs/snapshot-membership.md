...
- **The Field Mapping page also lists "Batch Uid," "Replacement Batch Uid," "Course ID," "Username," and "Move Cross-listed Enrollment"** as Learn Enrollments fields, none of which appear anywhere in the Header Override list — they cannot be populated from the flat file at all. "Batch Uid," "Course Batch Uid," and "User Batch Uid" all show fixed (non-configurable) sources labeled "Membership External Key," "Membership Course/Organization External Key," and "Membership User External Key" respectively — despite the similar naming, only Course Batch Uid and User Batch Uid are real, required fields (they're the same `external_course_key`/`external_organization_key` and `external_person_key` headers already documented above as "Course / Org Batch Uid" and "User Batch Uid"). "Batch Uid" itself, along with Course ID, Username, Replacement Batch Uid, and Move Cross-listed Enrollment, have no real header at all — Blackboard Learn has no separate membership batch UID concept beyond the course and person keys. Treat these "Membership ... External Key" Field Mapping labels as internal echoes of the already-documented required headers, not distinct fields or values you need to supply.
