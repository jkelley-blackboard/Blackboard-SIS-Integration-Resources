...
| Include In Roster | `roster_ind` | No | No | `Y` \| `N` | Flag indicating whether the user is displayed in the course roster. **Untested in Ultra course view; no associated GUI control found.** |
...
| Receive Email | `receive_email_ind` | No | No | `Y` \| `N` | Flag whether email is enabled for this user in this course. **Untested in Ultra course view; no associated GUI control found.** |
...
- **`roster_ind` (Include In Roster) and `receive_email_ind` (Receive Email) are a different, lesser case:** unlike the deprecated fields above, these aren't confirmed non-functional — their behavior in the Ultra course view is simply untested, and neither has a known corresponding GUI control to verify against. Treat as unconfirmed, not deprecated.
- **`external_course_key` (or `external_organization_key`) and `external_person_key` are the paired key values that determine a membership record** — together they identify which course-and-user combination the record describes.
- **The Field Mapping page also lists "Batch Uid," "Course Batch Uid," "User Batch Uid," "Replacement Batch Uid," "Course ID," "Username," and "Move Cross-listed Enrollment"** as Learn Enrollments fields. Of these, only Course Batch Uid and User Batch Uid are confirmed real (Required Yes, fixed sources tied to the `external_course_key`/`external_organization_key` and `external_person_key` headers already documented above as "Course / Org Batch Uid" and "User Batch Uid"). The rest — Batch Uid (fixed source "Membership External Key"), Course ID, Username, Replacement Batch Uid, and Move Cross-listed Enrollment — have no corresponding entry in the Header Override list and cannot be populated from the flat file. What "Membership External Key" actually refers to on the backend, and whether Learn has any separate membership-level batch UID beyond the course/person key pair, is not confirmed — treat as unknown rather than assumed either way.
