...
| Service Level Type | `service_level` | No | No | `F` \| `C` \| `S` (`R`, `T` not used) | **Deprecated.** Set automatically by the endpoint — do not supply, and never use it to define or change a record's type. Subjects, Programs, LOR, and system courses each have their own object spec/endpoint; do not attempt to represent them through the Course/Organization object. |
...
- **`data_source_key`** may be supplied by the integration configuration and does not need to appear in the file if so.
